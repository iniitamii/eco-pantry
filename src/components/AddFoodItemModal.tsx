...
// Add this constant near the top with CATEGORIES and UNITS
...
export function AddFoodItemModal({ onClose, onSuccess }: Props) {
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 style={{ fontFamily: "'Lora', serif" }} className="text-2xl font-semibold text-stone-800">Add to Pantry</h2>
            <p className="text-xs text-stone-400 mt-0.5">Track a new food item</p>
          </div>
        ...
        <form ref={formRef} action={handleSubmit} className="space-y-4">
          <div>
            ...
            <input name="name" required placeholder="e.g. Organic Whole Milk"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input name="quantity" type="number" min="0.1" step="0.1" required placeholder="1"
          ...
          <div>
            ...
            <select name="storageLocation" defaultValue="PANTRY"
              className="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-emerald-400">
          ...
          <div>
            ...
            <input name="expiryDate" type="date" required min={today}
              className="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-emerald-400" />
            {fieldError("expiryDate") && <p className="text-xs text-rose-500 mt-1">{fieldError("expiryDate")}</p>}
          </div>

          <div>
            <label className="block text-xs font-semibold text-stone-500 uppercase tracking-wide mb-1.5">Notes <span className="normal-case font-normal">(optional)</span></label>
            <textarea name="notes" rows={2} placeholder="e.g. Opened, store in fridge"
